...
if [ -n "$OLD_SNAPSHOT_UUIDS" ]; then
    echo "($(date)) | Ì†ΩÌ∑ë Deleting old snapshots for $SNAPSHOT_NAME..."
    for SNAPSHOT_UUID in $(echo "$OLD_SNAPSHOT_UUIDS" | tr ',' ' '); do
        echo "($(date)) | ‚û§ Removing snapshot: $SNAPSHOT_UUID"
        xe snapshot-destroy uuid="$SNAPSHOT_UUID"
    done
    echo "($(date)) | ‚úÖ Old snapshots deleted."
fi

# Create a new snapshot
echo "($(date)) | Ì†ΩÌ≥∏ Creating new snapshot: $SNAPSHOT_NAME..."
SNAPSHOT_UUID=$(xe vm-snapshot vm="$VM_UUID" new-name-label="$SNAPSHOT_NAME" 2>&1)

if [[ $? -ne 0 ]]; then
    echo "($(date)) | ‚ùå Snapshot creation failed! Error: $SNAPSHOT_UUID"
    exit 1
fi

echo "($(date)) | ‚úÖ Snapshot $SNAPSHOT_NAME created!"
